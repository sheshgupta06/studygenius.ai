...
async def stream_chat(
    ...
    doc_result = await db.execute(
        select(Document).where(
            Document.id == uuid.UUID(str(request.document_id)),
    ...
    if doc.status == "failed":
        raise HTTPException(
            ...
            detail="Document processing failed. Please re-upload the document.",
        )
    if doc.status == "uploading":
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Document is still uploading. Please wait.",
        )
